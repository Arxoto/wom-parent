...
本质是实现“抽象”的一种方式，可以类比为设计模式中的策略模式，由此引出思考：
    ...
    作如下约定：
        ...
        非实体类（主要作为逻辑承载，如下的纯函数策略模式的领域服务），使用接口（虚函数表）来保证重用；
    设想：
        不允许可变全局变量（非 final 的 static 变量），如此可强化降低函数副作用（有下一条规则的话可能也不是很有必要）；
        对于函数和方法，若内部有做修改（ Rust 的 mut 变量），必须显式标明为 mut 方法，否则缺省为 pure 函数（类似于 Rust 的闭包类型）；
        减少全局变量的使用，代价是需要手写框架来管理类全局变量的生命周期以及依赖注入（可参考 Bevy 的 ECS 框架实现）；
    参考 DDD 的思想：
